...
void generate_image_data_task(const Task *task,
  ...
  const FieldAccessor<READ_WRITE, ImageReduction::PixelField, image_region_dimensions, coord_t, Realm::AffineAccessor<ImageReduction::PixelField, image_region_dimensions, coord_t> > r(image, ImageReduction::FID_FIELD_R);
  const FieldAccessor<READ_WRITE, ImageReduction::PixelField, image_region_dimensions, coord_t, Realm::AffineAccessor<ImageReduction::PixelField, image_region_dimensions, coord_t> > g(image, ImageReduction::FID_FIELD_G);
  const FieldAccessor<READ_WRITE, ImageReduction::PixelField, image_region_dimensions, coord_t, Realm::AffineAccessor<ImageReduction::PixelField, image_region_dimensions, coord_t> > b(image, ImageReduction::FID_FIELD_B);
  const FieldAccessor<READ_WRITE, ImageReduction::PixelField, image_region_dimensions, coord_t, Realm::AffineAccessor<ImageReduction::PixelField, image_region_dimensions, coord_t> > a(image, ImageReduction::FID_FIELD_A);
  const FieldAccessor<READ_WRITE, ImageReduction::PixelField, image_region_dimensions, coord_t, Realm::AffineAccessor<ImageReduction::PixelField, image_region_dimensions, coord_t> > z(image, ImageReduction::FID_FIELD_Z);
  const FieldAccessor<READ_WRITE, ImageReduction::PixelField, image_region_dimensions, coord_t, Realm::AffineAccessor<ImageReduction::PixelField, image_region_dimensions, coord_t> > userdata(image, ImageReduction::FID_FIELD_USERDATA);
...
static int verifyImage(ImageDescriptor imageDescriptor,
  ...
  for(int y = 0; y < imageDescriptor.height; y++) {
    for(int x = 0; x < imageDescriptor.width; x++) {
      ...
      if(!verifyImageReduction(pixelID, (char*)"r", *expected++, r[x][y][Z])) {
        failures++;
      }
      if(!verifyImageReduction(pixelID, (char*)"g", *expected++, g[x][y][Z])) {
        failures++;
      }
      if(!verifyImageReduction(pixelID, (char*)"b", *expected++, b[x][y][Z])) {
        failures++;
      }
      if(!verifyImageReduction(pixelID, (char*)"a", *expected++, a[x][y][Z])) {
        failures++;
      }
      if(!verifyImageReduction(pixelID, (char*)"z", *expected++, z[x][y][Z])) {
        failures++;
      }
      if(!verifyImageReduction(pixelID, (char*)"userdata", *expected++, userdata[x][y][Z])) {
        failures++;
      }
...
int verify_composited_image_data_task(const Task *task,
  ...
  if(layer == 0) {
    ...
    const FieldAccessor<READ_ONLY, ImageReduction::PixelField, image_region_dimensions, coord_t, Realm::AffineAccessor<ImageReduction::PixelField, image_region_dimensions, coord_t> > r(image, ImageReduction::FID_FIELD_R);
    const FieldAccessor<READ_ONLY, ImageReduction::PixelField, image_region_dimensions, coord_t, Realm::AffineAccessor<ImageReduction::PixelField, image_region_dimensions, coord_t> > g(image, ImageReduction::FID_FIELD_G);
    const FieldAccessor<READ_ONLY, ImageReduction::PixelField, image_region_dimensions, coord_t, Realm::AffineAccessor<ImageReduction::PixelField, image_region_dimensions, coord_t> > b(image, ImageReduction::FID_FIELD_B);
    const FieldAccessor<READ_ONLY, ImageReduction::PixelField, image_region_dimensions, coord_t, Realm::AffineAccessor<ImageReduction::PixelField, image_region_dimensions, coord_t> > a(image, ImageReduction::FID_FIELD_A);
    const FieldAccessor<READ_ONLY, ImageReduction::PixelField, image_region_dimensions, coord_t, Realm::AffineAccessor<ImageReduction::PixelField, image_region_dimensions, coord_t> > z(image, ImageReduction::FID_FIELD_Z);
    const FieldAccessor<READ_ONLY, ImageReduction::PixelField, image_region_dimensions, coord_t, Realm::AffineAccessor<ImageReduction::PixelField, image_region_dimensions, coord_t> > userdata(image, ImageReduction::FID_FIELD_USERDATA);
...
static int verifyAccumulatorMatchesResult(ImageReduction &imageReduction, Image expected, ImageDescriptor imageDescriptor, HighLevelRuntime* runtime, Context context) {
  ...
  FutureMap futures = imageReduction.launch_task_composite_domain(VERIFY_COMPOSITED_IMAGE_DATA_TASK_ID, Legion::Runtime::get_runtime(), context, expected, totalSize, true);
...
//Point<2>(0, 0), Point<2>(imageDescriptor.width-1, imageDescriptor.height-1));
  ...
  std::vector<FieldID> attach_fields;
  attach_fields.push_back(ImageReduction::FID_FIELD_R);
  attach_fields.push_back(ImageReduction::FID_FIELD_G);
  attach_fields.push_back(ImageReduction::FID_FIELD_B);
  attach_fields.push_back(ImageReduction::FID_FIELD_A);
  attach_fields.push_back(ImageReduction::FID_FIELD_Z);
